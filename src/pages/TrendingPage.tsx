  ...
  useEffect(() => {
    const fetchSneakers = async () => {
      try {
        ...
        // For trending, we'll show the first 6 sneakers
        // In a real app, you might want to sort by popularity or newest first
        setSneakers(response.data.data.slice(0, 6));
      } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-mono-dark-600 dark:text-mono-light-600">
          Check out our most popular and latest sneakers
        </p>
      </div>
      ...
      {sneakers.length === 0 ? (
        <Alert type="info">No trending sneakers available right now.</Alert>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
