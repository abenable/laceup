  ...
  useEffect(() => {
    const fetchSneakers = async () => {
      try {
        ...
        // Filter for women's sneakers
        setSneakers(
          response.data.data.filter(
            (sneaker) => sneaker.category.toLowerCase() === "women"
          )
        );
      } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-mono-dark-600 dark:text-mono-light-600">
          Explore our collection of women's sneakers
        </p>
      </div>
      ...
      {sneakers.length === 0 ? (
        <Alert type="info">No women's sneakers available right now.</Alert>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
